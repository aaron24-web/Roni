...
const enlaceEstilo = ({ isActive }) => (isActive ? navButtonSelected : navButton);
...
export default function Layout({ perfil, onLogout }) {
    const { corteActivo, cargandoCorte, tickets, ticketActivoId, setTicketActivoId, crearNuevoTicket, cerrarTicket } = usePos();
        ...
        <div className="App">
            <header className="App-header">
                ...
                <div>
                    ...
                    <button onClick={onLogout} style={{ marginLeft: '15px' }}>Cerrar Sesión</button>
                </div>
            </header>
            ...
            <nav style={navStyles}>
                ...
                {esAdministrador(perfil) && enlacesAdmin.map(({ to, label }) => (
                    <NavLink key={to} to={to} style={enlaceEstilo}>{label}</NavLink>
                ))}
            </nav>
