-- 015: Los SERVICIOS no controlan existencias.
--
-- Bug corregido: `registrar_venta_completa` exigía stock en TODOS los renglones
-- sin mirar el tipo de producto. Como todo producto nace con su fila en
-- `inventario`, un servicio (fotocopias, impresiones, engargolado) creado con
-- cantidad inicial 0 era IMPOSIBLE de vender: "Stock insuficiente".
--
-- Ahora, para `tipo_producto = 'SERVICIO'` se saltan las tres operaciones de
-- inventario: validación de existencias, descuento y movimiento. El precio, la
-- promoción y el renglón de `detalleventa` no cambian: un servicio se cobra y
-- se audita igual que cualquier otro producto.
--
-- KIT se queda como está (descuenta su propio stock, como un artículo normal)
-- hasta decidir si los paquetes se arman físicamente o explotan sus
-- componentes al vender.
--
-- Copia de la versión de la migración 013 con ese único cambio.
...
begin
    ...
    loop
        ...
        -- Un servicio no tiene existencias: nada que validar ni descontar.
        controla_stock := coalesce(prod.tipo_producto, 'UNITARIO') <> 'SERVICIO';

        if controla_stock then
